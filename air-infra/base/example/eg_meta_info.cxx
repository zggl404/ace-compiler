...
struct OPR_INFO Opr_info[] = {
    {"FUNC_ENTRY", OPR_KIDS::FLEXIBLE, 0, PROP_SCF | PROP_STMT                         },
    {"PRAGMA",     0,                  0, PROP_LEAF | PROP_STMT | PROP_SYM | PROP_VALUE},
    {"DO_LOOP",    4,                  0, PROP_SCF | PROP_STMT                         },
    {"LDID",       0,                  0, PROP_LEAF | PROP_LOAD | PROP_SYM | PROP_TYPE },
    {"LABEL",      0,                  0, PROP_LEAF | PROP_LABEL                       },
    {"ADD",        2,                  0, PROP_EXPR | PROP_TYPE                        },
    {"CALL",       OPR_KIDS::FLEXIBLE, 0,
...
int main() {
  ...
  for (uint32_t i = 0; i < META_INFO::Num_domain_op(ID); ++i) {
    printf("  [%d] %s %d %x\n", i, META_INFO::Op_name(ID, i),
           META_INFO::Op_num_child(ID, i), META_INFO::Op_properties(ID, i));
