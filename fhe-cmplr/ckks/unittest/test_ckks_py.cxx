...
public:
  void Lower_add(NODE_PTR n_add) {
    ...
    NODE_PTR      new_blk = new_cntr->New_stmt_block(n_add->Spos());
    ctx.Push(Container()->Entry_node(), new_blk);
    ...
    ctx.Pop(Container()->Entry_node(), new_blk);

    Ofile() << "PY Lower After:" << std::endl;
    Ofile() << "====================================" << std::endl;
    new_blk->Print_tree(Ofile());
  }
