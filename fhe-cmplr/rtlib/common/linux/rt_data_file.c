...
bool Rt_data_fill(struct RT_DATA_FILE* file, void* buf, uint64_t sz) {
  IS_TRUE(file->_hdr._ent_type != DE_PLAINTEXT, "bad entry type");
  uint64_t ofst = DATA_FILE_PAGE_SIZE;
  while (sz > 0) {
    uint64_t size = (sz > DATA_FILE_READ_SIZE) ? DATA_FILE_READ_SIZE : sz;
    ssize_t  ret  = pread(file->_fd, buf, size, ofst);
    IS_TRUE(ret == size, "failed to fill data from file");
    if (ret <= 0) {
      break;
    }
    buf += ret;
