...
void Free_data(void* poly) {
  if ((Pt_mgr._file) && (!Rt_data_is_plaintext(Pt_mgr._file))) {
    Free_poly_data(poly);
  }
}

extern void Encode_float(void* plain, float* input, size_t len,
                         uint32_t sc_degree, uint32_t level);

void* Pt_from_msg(void* pt, uint32_t index, size_t len, uint32_t scale,
                  uint32_t level) {
  if (Rt_data_is_plaintext(Pt_mgr._file)) {
    pt = Pt_get(index, len, scale, level);
  } else {
    ...
    Encode_float(pt, data, len, scale, level);
  }
  return pt;
}
...
void* Pt_from_msg_ofst(void* pt, uint32_t index, size_t ofst, size_t len,
  ...
  uint64_t file_ofst =
      Rt_data_entry_offset(Pt_mgr._file, index, len * sizeof(float));
  IS_TRUE(file_ofst + (ofst + len) * sizeof(float) <= Pt_mgr._pt_size,
          "entry offset too large");
  float* data = (float*)&Pt_mgr._pt_buf[file_ofst + ofst * sizeof(float)];
  Encode_float(pt, data, len, scale, level);
  return pt;
}
