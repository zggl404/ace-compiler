...
static inline size_t Get_ciph_num_pq(CIPHERTEXT* ciph) {
  return Get_num_pq(Get_c0(ciph));
}
...
static inline void Init_ciphertext(CIPHERTEXT* res, uint32_t ring_degree,
  ...
  if (Get_poly_coeffs(c0) == NULL) {
    ...
    IS_TRUE(Get_rdgree(c0) == ring_degree && Poly_level(c0) >= num_primes,
            "unmatched ciphertxt");
    Set_poly_level(c0, num_primes);
  }
  if (Get_poly_coeffs(c1) == NULL) {
    Alloc_poly_data(Get_c1(res), ring_degree, num_primes, num_primes_p);
  } else {
    IS_TRUE(Get_rdgree(c1) == ring_degree && Poly_level(c1) >= num_primes,
            "unmatched ciphertxt");
    Set_poly_level(c1, num_primes);
  }
}
...
static inline void Copy_ciphertext(CIPHERTEXT* res, CIPHERTEXT* ciph) {
  ...
  Copy_poly(Get_c0(res), Get_c0(ciph));
...
static inline size_t Get_ciph_level(CIPHERTEXT* ciph) {
  POLYNOMIAL* c0 = Get_c0(ciph);
  IS_TRUE(Poly_level(c0) == Poly_level(Get_c1(ciph)),
          "unmatched level in ciph");
  return Poly_level(c0);
}
...
static inline CIPHERTEXT* Adjust_level(CIPHERTEXT* ciph1, CIPHERTEXT* ciph2,
  ...
  IS_TRUE(Num_p(Get_c0(ciph1)) == Num_p(Get_c0(ciph2)), "p prime not match");
...
//! @brief Get the number of p prime from ciphertext3
...
static inline size_t Get_ciph3_level(CIPHERTEXT3* ciph) {
  ...
  IS_TRUE(Poly_level(c0) == Poly_level(Get_ciph3_c1(ciph)) &&
              Poly_level(c0) == Poly_level(Get_ciph3_c2(ciph)),
          "unmatched level in ciph");
  return Poly_level(c0);
}
...
static inline void Init_ciphertext3_from_ciph(CIPHERTEXT3* res,
  ...
  res->_sf_degree      = sf_degree;
  res->_slots          = ciph->_slots;
  IS_TRUE(Poly_level(Get_c0(ciph)) == Poly_level(Get_c1(ciph)),
          "unmatched level in ciph");
...
static inline void Init_ciphertext3_from_ciph3(CIPHERTEXT3* res,
  ...
  res->_sf_degree      = sf_degree;
  res->_slots          = ciph->_slots;
  IS_TRUE(Poly_level(Get_ciph3_c0(ciph)) == Poly_level(Get_ciph3_c1(ciph)),
          "unmatched level in ciph");
  IS_TRUE(Poly_level(Get_ciph3_c0(ciph)) == Poly_level(Get_ciph3_c2(ciph)),
          "unmatched level in ciph");
...
static inline CIPHERTEXT3* Adjust_ciph3_level(CIPHERTEXT3* ciph1,
  ...
  IS_TRUE(Num_p(ciph1_c0) == Num_p(ciph2_c0), "p prime not match");
