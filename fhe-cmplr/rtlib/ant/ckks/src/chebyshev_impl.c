...
void Compute_degree_ps(VL_UI32* res, uint32_t n) {
  ...
  if (n <= UPPER_BOUND_PS) {
    Precomp_mlist = Populate_param_ps();
...
void Inner_eval_chebyshev_ps(CKKS_BTS_CTX* bts_ctx, CIPHERTEXT* out,
  ...
  Mul_ciphertext(out, out, qu, eval);
  Rescale_ciphertext(out, out, eval);
...
void Eval_chebyshev_ps(CKKS_BTS_CTX* bts_ctx, CIPHERTEXT* out, CIPHERTEXT* in,
  ...
  uint32_t    neg_1_level   = Get_ciph_level(t0);
  VALUE_LIST* neg_1_scalars = Alloc_value_list(I64_TYPE, neg_1_level);
  Encode_val(neg_1_scalars, -1.0, neg_1_level, Get_ciph_sfactor(t0),
             Get_ciph_prime_p_cnt(t0));
  for (uint32_t i = 2; i <= k; i++) {
    ...
    if (!(i & (i - 1))) {
      ...
      Mul_ciphertext(prod, t_ihalf_1, t_ihalf_1, eval);
      Add_ciphertext(t_j, prod, prod, eval);
      Rescale_ciphertext(t_j, t_j, eval);
      VALUE_LIST sub_scalars;
      Extract_value_list(&sub_scalars, neg_1_scalars, 0, Get_ciph_num_pq(t_j));
      Add_scalars(t_j, t_j, &sub_scalars, eval);
    } else {
      // non-power of 2
      if (i % 2 == 1) {
        // if i is odd
        ...
        CIPHERTEXT* t_ihalf_1 = (CIPHERTEXT*)PTR_VALUE_AT(t_list, i / 2 - 1);
        Mul_ciphertext(prod, t_ihalf_1, t_ihalf, eval);
        Add_ciphertext(t_j, prod, prod, eval);
        Rescale_ciphertext(t_j, t_j, eval);
        ...
        Mul_ciphertext(prod, t_ihalf_1, t_ihalf_2, eval);
        Add_ciphertext(t_j, prod, prod, eval);
        Rescale_ciphertext(t_j, t_j, eval);
        if (ihalf_1 == ihalf_2) {
          VALUE_LIST sub_scalars;
          Extract_value_list(&sub_scalars, neg_1_scalars, 0,
                             Get_ciph_num_pq(t_j));
          Add_scalars(t_j, t_j, &sub_scalars, eval);
        } else {
  ...
  if (FIXED_MANUAL) {
    ...
    for (size_t i = 1; i < k; i++) {
      ...
      while (level_diff > 0) {
        Modswitch_ciphertext(ti_1, eval);
        level_diff--;
      }
  ...
  for (uint32_t i = 1; i < m; i++) {
    ...
    Mul_ciphertext(prod, t2_i_1, t2_i_1, eval);
    Add_ciphertext(t2_i, prod, prod, eval);
    Rescale_ciphertext(t2_i, t2_i, eval);
    VALUE_LIST sub_scalars;
    Extract_value_list(&sub_scalars, neg_1_scalars, 0, Get_ciph_num_pq(t2_i));
    Add_scalars(t2_i, t2_i, &sub_scalars, eval);
  }
  ...
  for (uint32_t i = 1; i < m; i++) {
    ...
    Mul_ciphertext(prod, t2km1, t2_values[i], eval);
