...
void Init_ckks_parameters_with_prime_size(
  ...
  FMT_ASSERT(input_lev <= prime_len, "input_lev must not exceed prime count");
  param->_input_level    = input_lev;
...
void Print_param(FILE* fp, CKKS_PARAMETER* param) {
  ...
  VALUE_LIST* q_prime = Get_primes(Get_q(param->_crt_context));
  ...
  VALUE_LIST* p_prime = Get_primes(Get_p(param->_crt_context));
