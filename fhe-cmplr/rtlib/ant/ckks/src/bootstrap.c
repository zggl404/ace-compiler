...
CIPHERTEXT* Rotate_iteration(CIPHERTEXT* result, CKKS_BTS_CTX* bts_ctx,
  ...
  // later on)
  VALUE_LIST* digits = Alloc_precomp(Get_c1(result));
...
#pragma omp parallel for
  for (int32_t j = 0; j < giant_step; j++) {
    ...
    if (val != 0) {
      Fast_rotate_ext(rot_ciph, result, val, eval, digits, true);
    } else {
  ...
  for (int32_t i = 0; i < baby_step; i++) {
    ...
    PLAINTEXT sub_plain;
    Derive_plain(&sub_plain, plain, Poly_level(Get_c0(rot_ciph_0)),
                 Num_p(Get_c0(rot_ciph_0)));
    ...
    for (int32_t j = 1; j < giant_step; j++) {
      ...
      if ((giant + j) != (int32_t)(num_rot)) {
        ...
        Derive_plain(&sub_plain_j, plain_j, Poly_level(Get_c0(rot_ciph_j)),
                     Num_p(Get_c0(rot_ciph_j)));
    ...
    if (i == 0) {
      ...
      if (val != 0) {
        ...
        Copy_poly(Get_c0(reduce_ciph), Get_c0(inner));
        Mod_down(Get_c1(reduce_ciph), Get_c1(inner));
        ...
        Automorphism_transform(&temp_poly, Get_c0(reduce_ciph), precomp);
        Add_poly(&first, &first, &temp_poly);

        VALUE_LIST* inner_digits = Alloc_precomp(Get_c1(reduce_ciph));
        Fast_rotate_ext(temp_ciph, reduce_ciph, val, eval, inner_digits, FALSE);
        Add_ciphertext(outer, outer, temp_ciph, eval);

        Free_ciphertext(reduce_ciph);
        Free_precomp(inner_digits);
      } else {
        Copy_poly(&temp_poly, Get_c0(inner));
        Add_poly(&first, &first, &temp_poly);
        memset(Get_c0(inner)->_data, 0, Get_poly_mem_size(Get_c0(inner)));
  ...
  Add_poly(Get_c0(outer), Get_c0(outer), &first);
  Mod_down(Get_c0(result), Get_c0(outer));
  Mod_down(Get_c1(result), Get_c1(outer));
...
void Apply_double_angle_iterations(CIPHERTEXT* ciph, uint32_t num_iter,
  ...
  for (int32_t j = 1; j < r + 1; j++) {
    Mul_ciphertext(ciph, ciph, ciph, eval);
...
CIPHERTEXT* Eval_bootstrap(CIPHERTEXT* res, CIPHERTEXT* ciph,
  ...
  if (slots == m / 4) {
    ...
    Conjugate(conj_ciph, enc_ciph, eval);
