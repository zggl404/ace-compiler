...
NODE_PTR CKKS_PY_IMPL::New_py_add(NODE_PTR node, NODE_PTR new_ld0,
  ...
  py::module m_instantiator = py::module::import("instantiator_ckks");
  ...
  NODE_PTR                            body = cntr->New_stmt_block(node->Spos());
...
FUNC_SCOPE* CKKS_PY_IMPL::New_py_bts(NODE_PTR node, const SPOS& spos) {
  ...
  AIR_ASSERT_MSG(mul_lev, "missing level on bootstrap");
  ...
  py::object                          py_new_var = py::cast(new_var);
  pybind11::detail::str_attr_accessor bootstrap =
      m_instantiator.attr("bootstrap");
  bootstrap(py_fs, py_body, py_node, py_new_var, py_formal0, *mul_lev);
...
FUNC_SCOPE* CKKS_PY_IMPL::New_bts_func(NODE_PTR node, const SPOS& spos) {
  ...
  func_name += std::to_string(*mul_lev);
