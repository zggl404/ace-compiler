...
MODELS = ['AlexNet',
          ...
          'SqueezeNet',
          "VGG16"]
INDEXES = ['alexnet_cifar10',
           'resnet20_cifar10',
           'squeezenet_cifar10',
           'vgg16_cifar10']
ONNX_FILES = ['alexnet_cifar10.onnx',
              'resnet20_cifar10_pre.onnx',
              'squeezenet_cifar10.onnx',
              'vgg16_cifar10.onnx']
...
def generate_accuracy(raw_acc, ace_acc, table6_name, log):
    '''
    Generate accuracy table (Table 6 in paper)
    '''
    acc_gain = []
...
def main():
    ...
    table6_name = 'Table6'
    if image_num != 1000:
        table6_name += '-' + str(image_num) + '-ImagesOnly'
    table6_name += '.pdf'
    ace_cmplr = '/app/ace-compiler/release_omp/driver/fhe_cmplr'
    ...
    write_log(info, log)
    # generate_accuracy(raw_acc, ace_acc, table6_name, log)
    log.close()
    res_dir = '/app/ae_result'
    if os.path.exists(res_dir):
        shutil.copyfile(table6_name, os.path.join(res_dir, table6_name))
    return
