...
def ace_compile_and_run_onnx(cwd, cmplr_path, onnx_path, onnx_model, log, debug):
    ...
    src_dir = '/app/ace-compiler/fhe-cmplr/rtlib/ant/dataset/'
    ...
    cmds.extend(['-SIHE:relu_vr_def=2:relu_mul_depth=13', '-CKKS:hw=192', '-o', onnx_c])
    # options for detailed time info
    cmds.extend(['-O2A:ts', '-FHE_SCHEME:ts', '-VEC:ts:rtt', '-SIHE:ts:rtt'])
    ...
    cmds = ['time', '-f', '\"%e %M\"', 'c++', os.path.join(src_dir, matching_index + '.cxx')]
    cmds.extend(['-DRTLIB_SUPPORT_LINUX'])
    cmds.extend(['-I', os.path.join(cmplr_path, 'include')])
    cmds.extend(['-I', os.path.join(cmplr_path, 'rtlib/include')])
    cmds.extend(['-I', os.path.join(cmplr_path, 'rtlib/include/ant')])
    cmds.extend(['-O3', '-std=gnu++17'])
    ...
    cmds.append(os.path.join(cmplr_path, 'lib/libAIRutil.a'))
...
def test_perf_ace(cwd, cmplr_path, onnx_path, log, debug):    
    subprocess.run(["python3", "/app/scripts/cgo25_ae.py"])
    return
