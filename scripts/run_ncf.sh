...
ncf_graph_file="${onnx_file%onnx*}ncf.onnx.c"
ncf_trace_file="${onnx_file%onnx*}ncf.t"
...
if [[ "$onnx_file" == "cryptonet.onnx" ]]; then
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=4096 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=8192 -P2C:fp:df=/tmp/fhe_data.bin /app/onnx_file/${onnx_file}
elif [[ "$onnx_file" == "lenet_ax2.onnx" || "$onnx_file" == "lenet.onnx" ]]; then
  # lenet
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=16384 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=32768 -P2C:fp:df=/tmp/fhe_data.bin /app/onnx_file/${onnx_file}
elif [[ "$onnx_file" == "resnet20_cifar10.onnx" || "$onnx_file" == "resnet20_cifar10_ax2.onnx" ]]; then
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=32768 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=65536 -P2C:fp:df=/tmp/fhe_data.bin /app/onnx_file/${onnx_file}
else
  # vgg, alexnet, mobilenet, squeezenet
  time ./fhe_cmplr -VEC:fcount:dmo:ncf -P2C:df=/tmp/fhe_data.bin:fp -CKKS:sbm:tsbm:hw=192:q0=60:sf=59 /app/onnx_file/${onnx_file}
fi
...
mv ${trace_file} /app/ae_result/fhefusion/${ncf_trace_file}
cp ${graph_file} /app/test/
mv ${graph_file} /app/test/${ncf_graph_file}
