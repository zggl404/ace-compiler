...
if [[ "$onnx_file" == "cryptonet.onnx" ]]; then
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=4096 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=8192 -P2C:fp:df=/tmp/fhe_data.bin /app/model/${onnx_file}
elif [[ "$onnx_file" == "lenet_ax2.onnx" || "$onnx_file" == "lenet.onnx" ]]; then
  # lenet
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=16384 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=32768 -P2C:fp:df=/tmp/fhe_data.bin /app/model/${onnx_file}
elif [[ "$onnx_file" == "resnet20_cifar10.onnx" || "$onnx_file" == "resnet20_cifar10_ax2.onnx" ]]; then
  time ./fhe_cmplr -VEC:fcount:dmo:ncf:ms=32768 -CKKS:sbm:tsbm:hw=192:q0=60:sf=59:N=65536 -P2C:fp:df=/tmp/fhe_data.bin /app/model/${onnx_file}
else
  # vgg, alexnet, mobilenet, squeezenet
  time ./fhe_cmplr -VEC:fcount:dmo:ncf -P2C:df=/tmp/fhe_data.bin:fp -CKKS:sbm:tsbm:hw=192:q0=60:sf=59 /app/model/${onnx_file}
fi
